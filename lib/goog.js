/*!
 *
 * GSearch JavaScript Library v0.1
 * https://github.com/edravis/gsearch
 * 
 * This library include the Google JSAPI
 * https://www.google.com/jsapi
 *
 * Copyright 2014 Ruben Sospedra and Huballin Labs and other contributors
 * Released under the MIT license
 * https://github.com/edravis/GSearch/blob/master/LICENSE
 *
 */

(function(){
	// Invoke Google
	var createGInstance = function(){
		...
		l(google,function(a,b,c){function e(a){var b=a.split(".");if(2<b[x])throw H("Module: '"+a+"' not found!");"undefined"!=typeof b[1]&&(f=b[0],c.packages=c.packages||[],c.packages[t](b[1]))}var f=a;c=c||{};if(a instanceof Array||a&&"object"==typeof a&&"function"==typeof a[B]&&"function"==typeof a.reverse)for(var k=0;k<a[x];k++)e(a[k]);else e(a);if(a=M[":"+f]){c&&!c.language&&c.locale&&(c.language=c.locale);

			c&&"string"==typeof c.callback&&(k=c.callback,k.match(/^[[\]A-Za-z0-9._]+$/)&&(k=g.eval(k),c.callback=k));
			
			if((k=c&&null!=c.callback)&&!a.s(b))throw H("Module: '"+f+"' must be loaded before DOM onLoad!");k?a.m(b,c)?g[r](c.callback,0):a.load(b,c):a.m(b,c)||a.load(b,c)}else throw H("Module: '"+f+"' not found!");});K("google.load",google.load);
	...
	// Build the library above Google
	(function(root, factory){
		if(root === window){
			root.GSFactory = factory(root, {}, createGInstance()); 
		};
	})(this, function(root, GSFactory){
		var _google = window.google;
		GSFactory.ABOUT = {
			name: "GSearch",
			version: "0.1",
			author: "Ruben Sospedra",
			powered: "Huballin Labs",
			link: "https://github.com/edravis/GSearch"
		};
		
		GSFactory.init = function(model){
			// Model can be: {module, type, version, query, lang}
			
			if(!model){
				erroring("GSFactory INIT() needs a MODEL. Note that TYPE property is required.") };
			
			// Check required params
			if(!model.module){
				erroring("GS MODULE property required"); };
			if(!model.type){
				erroring("GS TYPE property required"); };
				
			// Check needed params
			model.version = model.version || "1";
			model.query 	= model.query || "";
			model.google 	= Object.create(_google);

			// Create the new GS object
			var _GS = Object.create(GS);
			_GS.model = model;
			_GS.load();
			return _GS;
		};
		
		var erroring = function(message){
			throw "[GS Prompter says] :: " + message;
		};

		// Create the prototyped instance
		var GS = {};
		...
		GS.get = function(property){
			return this.model[property];
		};
		GS.set = function(property, value){
			return this.model[property] = value;
		};
		

		// Working with the google instance

		GS.load = function(){
			this.get('google').load(this.get('module'), this.get('version'));
			this.init();
		};
		GS.init = function(){
			var me = this;
			switch(this.get('type')){
				case 'blogs':
					this.get('google').setOnLoadCallback(function(e){
						me.set('instance', new google[me.get('module')].BlogSearch());
						me.getNews();
					});
					break;
				case 'feeds':
					...
					this.get('google').setOnLoadCallback(function(e){
						var _g = me.get('google');
						me.set('instance', new _g[me.get('module')].Feed(me.composeRoute()));
						me.getFeeds();
					});
					break;
				case 'images':
					this.get('google').setOnLoadCallback(function(e){
						var _g 				= me.get('google'),
								_instance = new _g[me.get('module')].ImageSearch();

						_instance.setRestriction(
							_g.search.ImageSearch.RESTRICT_RIGHTS,
							_g.search.ImageSearch.RIGHTS_COMERCIAL_REUSE
						);

						_instance.setSearchCompleteCallback(me, me.getImages, null);
						_instance.execute(me.get('query'));
						me.set('instance', _instance);
					});
					break;
				case 'locals':
					this.get('google').setOnLoadCallback(function(e){
						var _g = me.get('google');
						me.set('instance', new _g[me.get('module')].LocalSearch());
						me.getNews();
					});
					break;
				case 'news':
					this.get('google').setOnLoadCallback(function(e){
						var _g 				= me.get('google'),
								_instance = new _g[me.get('module')].NewsSearch();
						_instance.setSearchCompleteCallback(me, me.getNews, null);
						_instance.execute(me.get('query'));
						me.set('instance', _instance);
					});
					break;
				case 'videos':
					this.get('google').setOnLoadCallback(function(e){
						var _g = me.get('google');
						me.set('instance', new _g[me.get('module')].VideoSearch());
						me.getNews();
					});
					break;
				}
		...
		GS.composeRoute = function(){
			var parameters = "";

			// Build the parameters
			parameters += "q=" + this.get('query');
			if(this.get('lang')){
				parameters += "&hl=" + this.get('lang'); };
			parameters += '&output=' + this.get('output');

			return 'https://news.google.com/news?' + parameters;
		};
		GS.cleanImages = function(raw){
			return {
				title: raw.titleNoFormatting,
				height: raw.height,
				width: raw.width,
				context: raw.originalContextUrl,
				unescapedUrl: raw.unescapedUrl,
				url: raw.url,
				mini: raw.tbUrl
			};
		};
		GS.cleanNews = function(raw){
			if(raw.relatedStories != undefined){
				var _relatedStories = [];
				for(var len in raw.relatedStories){
					_relatedStories.push(
						this.cleanNews(raw.relatedStories[len])
					);
				};
				raw.relatedStories = _relatedStories;
			};
			return {
				image: raw.image,
		...
		// Callbacks and response handlers

		GS.getFeeds = function(){
			var links	= [],
					me 		= this;

			this.get('instance').load(function(res){
				var entries = res.feed.entries;
				for(var len = entries.length; len--; len >= 0){
					var entry = entries[len];
					links.push({ title: entry.title, url: entry.link.split('&url=').pop() });
				};

				me.set('results', links);
			});
		};
		GS.getImages = function(){
			var _results 	= [],
					_images 	= this.get('instance').results;

			for(var len in _images){
				_results.push(this.cleanImages(_images[len]));
			};

			this.set('results', _results);
		};
		GS.getNews = function(){
			var _results 	= [],
					_news 		= this.get('instance').results;

			for(var len = _news.length; len--; len >= 0){
				_results.push(this.cleanNews(_news[len]));
			};
